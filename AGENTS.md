...
**Frontend** (`frontend/`): React 19 + TypeScript + Vite SPA styled with PrimeReact 11 (compound components + `@primeuix/themes` Aura preset). Provider stack in `src/main.tsx`: `PrimeReactProvider` (from `@primereact/core/config`) → `ThemeProvider` (from `@primereact/core/theme`) → `AuthProvider` → `ToastProvider` + `App`. Routes live in `src/router/`: public (outside `RouteGuard`) are `/` (HomePage: explore + login/dashboard), `/login`, `/auth/callback`, and the `/explore/*` browse pages (`/explore`, `/explore/venues`, `/explore/events`, `/explore/calendar`, `/explore/venues/:venueId`, `/explore/events/:eventId`). Explore routes are wrapped in `AppLayout` (sidebar + topbar visible) but sit outside `RouteGuard` so they're accessible without auth. Authenticated routes sit behind `RouteGuard` + `AppLayout`: `/dashboard`, `/profile`, `/my-schedule`, `/manage-events`, `/manage-users`, the five CRUD pages, `/forms`, `/forms/facility/:facilityId`, `/forms/builder/:facilityId`. The sidebar nav (`src/layout/nav.ts`) is role-filtered via rank-based `hasRole`: MEMBER sees Dashboard, Explore Venues, Explore Events, Explore Calendar, Signup Forms, My Schedule (+ Profile in the sidebar footer); COACH adds Manage Events; FACILITY_MANAGER adds Frequencies/Facilities/Events/Venues/Schedules/Manage Users; WEB_ADMIN sees everything. The topbar (`app-topbar`) shows the company name/logo (centered via `app-topbar-center`) and a right-aligned user chip (avatar above name, clickable to profile), sign-out button, and theme switch. API calls go through `src/api/client.ts` (Bearer header, single 401 → refresh → retry, then a `swimlane:auth-unauthorized` event that signs the user out). The login flow passes the SPA's own origin via `?frontend_url=` so the OAuth callback always returns to the correct port.
...
- Frontend: PrimeReact 11 Sidebar compound components (`Sidebar.Layout`, `Sidebar.Main`) need explicit CSS in `index.css` (`[data-scope="sidebar"][data-part="layout"]` and `[data-scope="sidebar"][data-part="main"]`) to ensure the flex layout works. The runtime theme CSS can be clobbered by primeflex or custom overrides — without these rules the main content area may not render beside the sidebar.
