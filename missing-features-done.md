...
## Phase G — Facility Manager: manage coach accounts (G.1–G.7 complete)
...
- **G.7** (`aaaad9a`): one-line nav insertion — `Manage Users` (`pi-users-cog`, `/manage-users`, `FACILITY_MANAGER`) between Schedules and Signup Forms. Nav filtering is automatic via `AppLayout`'s `hasRole(item.requiredRole)`; the page itself also enforces the backend's `facility_manager_role` dependency when routed in G.8.
...
- Only G.1–G.7 are in scope so far (the user's requests). G.8 (router — which also unblocks the SchedulesPage member picker) remains on the todo. The G.1.3 `Literal["coach", "member"]`, the G.1.1 senior-role 403, G.1.4's assign bound, and G.1.5's delete bound together satisfy G.1.6 — a facility manager can never assign or manage senior roles.
