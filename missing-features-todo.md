...
- [ ] **G.5** — `frontend/src/api/types.ts` — `ManagedUser`, `ManagedUserInput`. Commit. (Types already landed with G.4 — a required dependency of the wrappers; ticking is a formality.)
...
- [ ] **H.1** — Reuse `UserRoutes` with `admin_role` guard for `facility_manager` + `web_admin` role assignment:
  - [ ] **H.1.1** — `GET /users?role=facility_manager` (`admin_role` only). Commit.
